...
// #define SCUDO_TRACING_PRINT
#define DEFAULT_SAMPLING_RATE 1000
#define DEFAULT_ALLOCA_THRESHOLD 1000
...
static uptr old_sa_handler = 0;
static bool old_sa_handler_not_set = true;
static bool handler_not_set = true;

static bool in_slow_start = true;
static uint64_t alloca_num = 0; // allocation of short granules

static uint64_t sampling_rate = DEFAULT_SAMPLING_RATE;
static uint64_t alloca_threshold = DEFAULT_ALLOCA_THRESHOLD;
static int sampling_skip = 0;
...
public:
  ...
  void init() {
    ...
    const char *env_sampling_rate = getenv("SCUDO_SAMPLING_RATE");
    if (env_sampling_rate) {
        sampling_rate = strtoull(env_sampling_rate, NULL, 10);
        Printf("SCUDO_SAMPLING_RATE set to: %lu\n", sampling_rate);
    } else {
        Printf("SCUDO_SAMPLING_RATE not set, using default value: %lu\n", sampling_rate);
    }

    const char *env_alloca_threshold = getenv("SCUDO_ALLOCA_THRESHOLD");
    if (env_alloca_threshold) {
        alloca_threshold = strtoull(env_alloca_threshold, NULL, 10);
        Printf("SCUDO_ALLOCA_THRESHOLD set to: %lu\n", alloca_threshold);
    } else {
        Printf("SCUDO_ALLOCA_THRESHOLD not set, using default value: %lu\n", alloca_threshold);
    }
        ...
        if (UNLIKELY(useMemoryTagging<AllocatorConfig>(Options))) {
          if (ClassId) {
            resizeTaggedChunk(reinterpret_cast<uptr>(OldTaggedPtr) + OldSize,
                              reinterpret_cast<uptr>(OldTaggedPtr) + NewSize,
                              OldSize, NewSize, untagPointer(BlockEnd));
...
private:
  ...
  initChunkWithMemoryTagging(const uptr ClassId, const Chunk::Origin Origin,
    ...
    if (LIKELY(ClassId)) {
      ...
      if (getChunkFromBlock(BlockUptr, &PrevUserPtr, &Header) &&
        ...
        uptr PrevSize = Header.SizeOrUnusedBytes;
        uptr PrevEnd = TaggedUserPtr + PrevSize;
        const uptr NextPage = roundUp(TaggedUserPtr, getPageSizeCached());
        if (NextPage < PrevEnd && loadTag(NextPage) != NextPage)
          PrevEnd = NextPage;
        TaggedPtr = reinterpret_cast<void *>(TaggedUserPtr);
        // resizeTaggedChunk before zeroing the content may cause intra-granule OOB
        ...
        resizeTaggedChunk(PrevEnd, TaggedUserPtr + Size, PrevSize, Size, BlockEnd);
      } else {
  ...
  NOINLINE void *retagBlock(const Options &Options, void *TaggedPtr, void *&Ptr,
    ...
    #ifdef SCUDO_DEBUG_PRINT
    Printf("[retagBlock] TaggedPtr: %p, Size: %lu\n", TaggedPtr, Size);
    #endif
  ...
  void checkSignalHanlder() {
    struct sigaction old_action;
    sigaction(SIGSEGV, NULL, &old_action);
    
    if (old_sa_handler_not_set) {
      old_sa_handler = reinterpret_cast<uptr>(old_action.sa_handler);
      old_sa_handler_not_set = false;
    } else {
      handler_not_set = (reinterpret_cast<uptr>(old_action.sa_handler) == old_sa_handler);
    }

    #ifdef SCUDO_DEBUG_PRINT
    Printf("sa_handler: %p; handler_not_set: %d\n", old_action.sa_handler, handler_not_set);
    #endif
  }

  uint64_t randSamplingSkip() {
    uint64_t rand_value = (static_cast<uint64_t>(rand()) % (2 * sampling_rate)) + 1;
  ...
  bool wantToSample() {
    ...
    sampling_skip = randSamplingSkip();
  ...
  void *prepareTaggedChunk(void *Ptr, uptr Size, uptr ExcludeMask,
    ...
    // allocation.
    #ifdef SCUDO_DEBUG_PRINT
		Printf("[prepareTaggedChunk] Ptr: %p, Size: %lu\n", Ptr, Size);
    #endif
    storeTag(reinterpret_cast<uptr>(Ptr) - archMemoryTagGranuleSize());

    uptr TaggedBegin, TaggedEnd;
    uptr PaddedGranuleSize = Size % archMemoryTagGranuleSize();
    #ifdef SCUDO_DEBUG_PRINT
		Printf("PaddedGranuleSize = %lu\n", PaddedGranuleSize);
    #endif
    ...
    bool not_set_tripwire = (handler_not_set || (wantToSample() == false));
		
    if (PaddedGranuleSize == 0 || not_set_tripwire) {
      // If the size of the allocation is 16-byte aligned, or if it is zero
      setRandomTag(Ptr, Size, ExcludeMask, &TaggedBegin, &TaggedEnd);
    } else {
      if (in_slow_start) {
        alloca_num++;

        if (alloca_num >= alloca_threshold) {
          in_slow_start = false;
        }
      }

      if (Size > archMemoryTagGranuleSize()) {
        #ifdef SCUDO_DEBUG_PRINT
        Printf("Branch 1, size %lu\n", Size);
        #endif
				uptr TaggedLastGranule;
        uptr RandomTagSize = Size - PaddedGranuleSize;
        setRandomTag(Ptr, RandomTagSize, ExcludeMask, &TaggedBegin, &TaggedLastGranule);
        TaggedEnd = TaggedLastGranule + archMemoryTagGranuleSize();
        #ifdef SCUDO_DEBUG_PRINT
				Printf("TaggedBegin: %p, TaggedEnd: %p\n", reinterpret_cast<void *>(TaggedBegin), reinterpret_cast<void *>(TaggedEnd));
        #endif
        // Set the tag of the last granule to padded granule size
        TaggedLastGranule = addFixedTag(untagPointer(TaggedLastGranule), PaddedGranuleSize);
				#ifdef SCUDO_DEBUG_PRINT
        Printf("TaggedLastGranule: %p\n", reinterpret_cast<void *>(TaggedLastGranule));
        #endif
        storeTag(TaggedLastGranule);
        // Store the random tag in the last byte of the padded granule
        *reinterpret_cast<u8 *>(TaggedLastGranule + archMemoryTagGranuleSize() - 1) = extractTag(TaggedBegin);
				#ifdef SCUDO_DEBUG_PRINT
        Printf("Branch 1 end\n");
        #endif

        #ifdef SCUDO_TRACING_PRINT
          Printf("SCUDO_DEBUG_TRACING tagging %p %lu\n", reinterpret_cast<void *>(untagPointer(TaggedLastGranule)), Size);
        #endif
      } else {
        #ifdef SCUDO_DEBUG_PRINT
        Printf("Branch 2, size: %lu\n", Size);
        #endif
        // The padded granule is the only granule
        TaggedBegin = selectRandomTag(reinterpret_cast<uptr>(Ptr), ExcludeMask);
        TaggedEnd = TaggedBegin + archMemoryTagGranuleSize();
        #ifdef SCUDO_DEBUG_PRINT
				Printf("TaggedBegin: %p, TaggedEnd: %p\n", reinterpret_cast<void *>(TaggedBegin), reinterpret_cast<void *>(TaggedEnd));
        #endif
        uint8_t Tag = extractTag(TaggedBegin);
        // Set the tag of the last granule to padded granule size
        uptr TaggedLastGranule = addFixedTag(reinterpret_cast<uptr>(Ptr), PaddedGranuleSize);
        #ifdef SCUDO_DEBUG_PRINT
				Printf("TaggedLastGranule: %p\n", reinterpret_cast<void *>(TaggedLastGranule));
        #endif
        storeTag(TaggedLastGranule);
        // Store the random tag in the last byte of the padded granule
        *reinterpret_cast<u8 *>(TaggedLastGranule + archMemoryTagGranuleSize() - 1) = Tag;
        #ifdef SCUDO_DEBUG_PRINT
				Printf("Branch 2 end\n");
        #endif
  ...
  void resizeTaggedChunk(uptr OldPtr, uptr NewPtr, uptr OldSize, uptr NewSize,
                         uptr BlockEnd) {
    #ifdef SCUDO_DEBUG_PRINT
    Printf("[resizeTaggedChunk] OldPtr: %p, NewPtr: %p, OldSize: %lu, NewSize: %lu\n",
           reinterpret_cast<void *>(OldPtr), reinterpret_cast<void *>(NewPtr),
           OldSize, NewSize);
    #endif
    ...
    // For the last granule in the old allocation, we need to set the tag to the random tag
    if ((handler_not_set == false) && (OldSize % archMemoryTagGranuleSize() > 0)) {
      // If resizeTaggedChunk is called by allocate, all granules are already retagged by retagBlock
      // If resizeTaggedChunk is called by reallocate, we need to assign the last granule with the correct tag
      storeTag(RoundOldPtr - archMemoryTagGranuleSize());
    }

    uptr RoundNewPtr;
    // Attention: **RoundOldPtr and NewPtr must have the same tag**
    #ifdef SCUDO_DEBUG_PRINT
    Printf("RoundOldPtr: %p, NewPtr: %p\n", reinterpret_cast<void *>(RoundOldPtr), reinterpret_cast<void *>(NewPtr));
    #endif
    if ((RoundOldPtr >> 56) != (NewPtr >> 56)) {
      Printf("SCUDO DEBUG: something is wrong in resizeTaggedChunk. RoundOldPtr: %p, NewPtr: %p\n", reinterpret_cast<void *>(RoundOldPtr), reinterpret_cast<void *>(NewPtr));
    }
    if (RoundOldPtr >= NewPtr) {
      ...
      #ifdef SCUDO_DEBUG_PRINT
      Printf("Branch 1. RoundNewPtr: %p\n", reinterpret_cast<void *>(RoundNewPtr));
      #endif
    } else {
      ...
      #ifdef SCUDO_DEBUG_PRINT
      Printf("Branch 2. RoundNewPtr: %p\n", reinterpret_cast<void *>(RoundNewPtr));
      #endif
    }

    // Tag the last granule of the allocation if NewSize if not 16-byte aligned
    // is NewSize is 0, we only store the end marker.
    uptr PaddedGranuleSize = NewSize % archMemoryTagGranuleSize();
    #ifdef SCUDO_DEBUG_PRINT
    Printf("PaddedGranuleSize: %lu\n", PaddedGranuleSize);
    #endif
    ...
    if ((PaddedGranuleSize > 0) && (not_set_tripwire == false)) {
      if (in_slow_start) {
        alloca_num++;
      ...
      uptr TaggedLastGranule = addFixedTag(untagPointer(RoundNewPtr - archMemoryTagGranuleSize()), PaddedGranuleSize);
      storeTag(TaggedLastGranule);
      #ifdef SCUDO_DEBUG_PRINT
      Printf("TaggedLastGranule: %p\n", reinterpret_cast<void *>(TaggedLastGranule));
      #endif
      // store the random tag in the last byte of the padded granule
      *reinterpret_cast<u8 *>(TaggedLastGranule + archMemoryTagGranuleSize() - 1) = extractTag(RoundOldPtr);

      #ifdef SCUDO_TRACING_PRINT
        Printf("SCUDO_DEBUG_TRACING retagging %p %lu\n", reinterpret_cast<void *>(untagPointer(TaggedLastGranule)), NewSize);
      #endif
    }
